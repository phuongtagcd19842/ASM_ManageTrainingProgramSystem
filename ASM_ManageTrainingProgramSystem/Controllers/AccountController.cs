...
namespace ASM_ManageTrainingProgramSystem.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        [HttpPost]
        [Authorize(Roles = "Admin")]
        public async Task<ActionResult> CreateTrainer(TrainerRegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    var TrainerInfo = new TrainerInfo
                    {
                        ...
                        ExternalOrInternal = model.ExternalOrInternal,
        ...
        public async Task<ActionResult> CreateTrainee(TraineeRegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    return RedirectToAction("Index", "TraineesList");
                }
                AddErrors(result);
            }
            // If we got this far, something failed, redisplay form
